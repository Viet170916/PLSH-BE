...
yêu cầu hãy tạo cho tôi 1 dữ liệu chuỗi dạng json có cấu trúc như này {key: string, value:string|number}[], khi mà người dùng nhập promt yêu cầu làm một việc gì đấy thì hãy tạo teo yêu cầu của người dùng và đưa dữ liệu đó vào
type Book = {
  id: number;
  title: string;
  description: string;
...
sau đó chuyển dữ liệu này thành kiểu trả ra 1 dữ liệu chuỗi dạng json có cấu trúc như này {mesage:string,data:{key: string, value:string|number}[]}, 
với key là key của type Book với việc là chuỗi key này có thể nest sâu vào cả trong các thuộc tính là object hay mảng, value là giá trị mà bạn được người dùng yêu cầu phân tích và chỉ có thể để là number hoặc string hoặc boolean hoặc null, không được để là object hay mảng, nếu thuộc tính là object hay mảng thì hãy để key là giá trị nested đến các thuộc tính con đó,
